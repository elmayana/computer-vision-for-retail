...
WEEKDAY_CUSTOMERS = 200
WEEKEND_CUSTOMERS = 900   # 4.5x multiplier
...
STORE_PAIRS   = {
    "Store_A_Entry": "Store_A_Exit",
    "Store_B_Entry": "Store_B_Exit",
}
...
def generate_journey():
    ...
    MANDATORY: Start at entrance → visit at least 1 corridor/walkway AND 1 store → exit
    
    Structure:
    - Entrance
    - Corridor/Walkway (required)
    - Store (entry + exit pair, required)
    ...
    # Guarantee at least 1 store (entry + exit pair)
    store_entry = random.choice(STORE_ENTRIES)
    journey.append(store_entry)
    journey.append(STORE_PAIRS[store_entry])
    used_stores.add(store_entry)
    
    # Optional: add 1-2 more zones for variety
    n_additional = random.randint(0, 2)
    attempts = 0
    while len(journey) - 2 < 3 + n_additional and attempts < 20:  # -2 to exclude entrance/exit pair
        attempts += 1
        candidates = [z for z in MIDDLES if z not in journey and z not in list(STORE_PAIRS.values())]
        if not candidates:
            break
        next_zone = random.choice(candidates)

        if next_zone in STORE_ENTRIES and next_zone not in used_stores:
            journey.append(next_zone)
            journey.append(STORE_PAIRS[next_zone])
            used_stores.add(next_zone)
        elif next_zone not in list(STORE_PAIRS.values()):
            journey.append(next_zone)
